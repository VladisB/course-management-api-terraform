...
resource "aws_codebuild_project" "api_build" {
  badge_enabled  = false
  build_timeout  = 60
  name           = "course-management-api-build-stage-${var.stage}"
  ...
  artifacts {
    ...
    name                   = "course-management-api-build-${var.stage}"
...
resource "aws_codebuild_project" "api_test" {
  badge_enabled  = false
  build_timeout  = 60
  name           = "course-management-api-test-stage-${var.stage}"
  description    = "CodeBuild project for Course Management API ${var.stage} environment."
  queued_timeout = 480
  service_role   = aws_iam_role.codebuild_role.arn
  ...
  artifacts {
    encryption_disabled    = false
    name                   = "course-management-api-test-${var.stage}"
    override_artifact_name = false
    packaging              = "NONE"
    type                   = "CODEPIPELINE"
  }
...
resource "aws_codebuild_project" "api_migration" {
  badge_enabled  = false
  build_timeout  = 60
  name           = "course-management-api-migration-stage-${var.stage}"
  description    = "CodeBuild project for Course Management API ${var.stage} environment."
  queued_timeout = 480
  service_role   = aws_iam_role.codebuild_role.arn
  ...
  artifacts {
    encryption_disabled    = false
    name                   = "course-management-api-migration-${var.stage}"
    override_artifact_name = false
    packaging              = "NONE"
    type                   = "CODEPIPELINE"
  }
